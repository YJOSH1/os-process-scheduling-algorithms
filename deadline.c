/* 
    Author: Yash Joshi
    Description: implements Deadline-Based scheduling algorithm
*/
...
//function to find process in the process queue with the shortest deadline
int findProcessWithShortestDeadline() {
    //variable that tracks index of process in queue with shortest deadline
    int shortestDeadlineIndex = 0;

    //iterate through every process in the process queue and check deadline
    for (int i = 0; i < numOfProcesses; i++) {
        //if process i's deadline is less than the deadline of the process that has the index stored in shortestdeadlineindex var and its deadline is greater than zero assign index to variabel
        if (processQueue[i].deadlineTime < processQueue[shortestDeadlineIndex].deadlineTime && processQueue[i].deadlineTime > 0) {
            shortestDeadlineIndex = i;
        }
    }

    //return index of shortest deadline 
    return shortestDeadlineIndex;
}

//function to move the process with the shortest deadline to the front of the queue
void moveProcessToFrontOfQueue(int processIndex) {
    //if the index of the process with shortest deadline is 0 then no need to move process as it is already at the front so exit function
    if (processIndex != 0) {
        //create temporary pcb to hold PCB
        pcb_t transit = processQueue[processIndex];
        //set process currently at the front of the queue to ready as it is being preempted and print to cmd line it is preempted
        processQueue[qStart].state = READY;
        printf("Time %i: %s has been preempted.\n", currentTime, processQueue[qStart].process_name);

        //iterate through array and move each pcb up one index until it reaches the index of the process that is being moved to front of queue
        for (int i = processIndex; i > 0; i--) {
            processQueue[i] = processQueue[i - 1];
        }

        //make start position of queue equal to process with shortest deadline
        processQueue[qStart] = transit;
        //set that process to running and print to cmd line it is running
...
void deadline(FILE *outFile) {
    ...
    printf("STARTING DEADLINE-BASED SCHEDULING ALGORITHM\n");
    printf("----------------------------------------------------\n");
    sleep(1);
    ...
    while (completedProcesses < numOfProcesses) {
        ...
        //finds process in process queue with shortest deadline and moves its PCB to the front of the queue to be executed
        moveProcessToFrontOfQueue(findProcessWithShortestDeadline());
        ...
        if (processQueue[qStart].remainingTime == processQueue[qStart].serviceTime && processQueue[qStart].state == READY && qStart != qEnd) {
