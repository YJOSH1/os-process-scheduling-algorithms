/* 
    Author: Yash Joshi
    Description: implements Round Robin scheduling algorithm
*/
...
//function to move process to end of queue when it has been preempted
//see global.h for detailed description
void moveProcessToEndOfQueue() {
    pcb_t transit = processQueue[qStart];
    transit.state = READY;
    transit.timeQuantum = 2;

    for (int i = 0; i < qEnd - 1; i++) {
        processQueue[i] = processQueue[i + 1];
    }
    
    processQueue[qEnd - 1] = transit;
}
...
void roundRobin(FILE *outFile) {
    ...
    printf("STARTING ROUND ROBIN SCHEDULING ALGORITHM\n");
    printf("-----------------------------------------\n");
    sleep(1);
    ...
    while (completedProcesses < numOfProcesses) {
        ...
        if (processQueue[qStart].remainingTime == 0) {
            ...
            //see global.h for detailed function description
            calculateResults();
            ...
            //remove finished process PCB from process queue array, ensures process information is only held while process is active in the system
        ...
        //if process isn't finished execution then check if it has used all its allocated execution time and needs to be preempted and that there are other processes in the ready queue
        } else if (processQueue[qStart].timeQuantum == 0 && qStart != qEnd - 1) {
            //print to cmd line process has been preempted
            printf("Time %i: %s has been preempted.\n", currentTime, processQueue[qStart].process_name);
            sleep(1);
            // move process from front of queue to end of queue
            moveProcessToEndOfQueue();
        //if process is the only one in the queue then reset time quantum and run again
        } else if (processQueue[qStart].timeQuantum == 0 && qStart == qEnd - 1) {
            processQueue[qStart].timeQuantum = 2;
        }

        //check if process is running for the first time in that specific iteration
        if (processQueue[qStart].state == READY && qStart != qEnd && processQueue[qStart].timeQuantum == 2) {
            ...
            //print to cmd line process is in the running state
            printf("Time %i: %s is in the running state.\n", currentTime, processQueue[qStart].process_name);
            sleep(1);
        }
        
        //run process by decrementing time quantum and remaining time
        if (qStart != qEnd) {
            processQueue[qStart].remainingTime--;
            processQueue[qStart].timeQuantum--;
        }
